...
as described in Section~\ref{sec:prep}.\todo{can we say "running applications more than 5 times did not improve our results, so we chose 5 as a balance"?}
...
We use the vendor-specific compiler on each platform to compile all applications except OpenMC (using GNU compilers). For each proxy/parent pair, we use the same input problem or parameters, where possible. In cases where we cannot run the same problem, we use the closest matching problem available\todo{defined how?}, and in all cases we size both proxy and parent application problems to use about 50\% of the available memory. 
...
connects to every process (rank) in each application, in order to collect node related performance counter data. We carefully examined all of the available performance events on each hardware platform\todo{to what end?}. We functionally tested to ensure that at a minimum, plausible data was returned for each event. We eliminated events that were returning no data or unstable (\ie vastly varying) data across application runs. Finally, we collected more than 500 hardware events for each application on the platform Intel Skylake. 
